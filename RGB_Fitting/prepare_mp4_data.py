...
if __name__ == '__main__':
    ...
    parser.add_argument('--template_mp4',
                        type=str,
                        default='../mp4/c_2.mp4',
                        help='template mp4')
    parser.add_argument('--driven_mp4',
                        type=str,
                        default='../mp4/c_1.mp4',
                        help='driven mp4')
    ...
    fnames = [
        args.template_mp4, args.driven_mp4
    ]
    mask = True
    for fn in fnames:
        tic = time.time()
        basename = os.path.basename(fn)
        input_data = dataset_op.get_mp4_data(fn, mask)
        if input_data is None:
            continue

        torch.save(input_data, os.path.join(args.output_dir, f'{basename}.pt'))
        mid = input_data['img'].shape[0] // 2
        input_img = tensor2np(input_data['img'][mid: mid + 1, :, :, :])
        if mask:
            skin_img = tensor2np(input_data['skin_mask'][mid: mid + 1, :, :, :])
            skin_img = img3channel(skin_img)
            parse_mask = tensor2np(input_data['parse_mask'][mid: mid + 1, :, :, :], dst_range=1.0)
            parse_img = draw_mask(input_img, parse_mask)
        gt_lm = input_data['lm'][mid, :, :].detach().cpu().numpy()
        ...
        if mask:
            ...
            combine_img = np.concatenate([input_img, lm_img], axis=1)
